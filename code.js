/*window.addEventListener("DOMContentLoaded", () => {
   ...
   //The transition Animtions and all that
    ...
    let bgContainer = document.querySelector("main .section-3 .bg")
    let amount = 1200
    let colors = ["#00ff94","#112d4e","#ff2e63","#50c4ed"]
...
headerIcon.addEventListener("click", () => {
    ...
    asideItems[0].addEventListener("click", () => { 
        window.scrollTo(0, 0)
    })
    asideItems[1].addEventListener("click", () => { 
        window.scrollTo(0, 1350)
    })    
    asideItems[2].addEventListener("click", () => { 
        window.scrollTo(0, 2120)
    })
    asideItems[3].addEventListener("click", () => { 
        window.scrollTo(0, 2650)
    })
    ...
    window.addEventListener ("scroll", () => {
        ...
        const removeAll = (v1,v2,v3) => {
            eval(`document.querySelector (".nav-indicators-wrapper").classList.remove("active-${v1}");
            document.querySelector (".nav-indicators-wrapper").classList.remove("active-${v2}");
            document.querySelector (".nav-indicators-wrapper").classList.remove("active-${v3}")`)
        }

        if (window.scrollY > 2600) {
            removeAll(1,2,3)
            document.querySelector (".nav-indicators-wrapper").classList.add("active-4")
        }
        else if (window.scrollY > 2100) {
            removeAll(1,2,4)
            document.querySelector (".nav-indicators-wrapper").classList.add("active-3")
        }
        else if (window.scrollY > 1330) {
            removeAll(1,3,4)
            document.querySelector(".nav-indicators-wrapper").classList.add("active-2")
        }
        else if (window.scrollY > -5) {
            removeAll(2,3,4)
            document.querySelector(".nav-indicators-wrapper").classList.add("active-1")
            
        }
    })

    // The gallery scroll thing
    ...
    for (c in Array.from(images)) {
         Array.from(images)[c].classList.add(`img-${c}`)
    }

    rgtBtn.addEventListener("click", () => {
         
         currentImage < images.length  ? currentImage ++ : "Nothing will happen"
         document.querySelector(".gallery-wrapper .image-wrapper").scrollTo(currentImage * 300,0)
    })
    
    lftBtn.addEventListener("click", () => {               
         currentImage > 0 ? currentImage -- : "Nothing will happen"
         document.querySelector(".gallery-wrapper .image-wrapper").scrollTo(currentImage * 300,0)
    })
...
window.addEventListener("DOMContentLoaded", () => {
    ...
    window.addEventListener("scroll", () => {
        const removeAll = (v1, v2, v3) => {
            document.querySelector(".nav-indicators-wrapper").classList.remove(`active-${v1}`);
            document.querySelector(".nav-indicators-wrapper").classList.remove(`active-${v2}`);
            document.querySelector(".nav-indicators-wrapper").classList.remove(`active-${v3}`);
        };
